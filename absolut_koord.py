...
# Способ В.Г. Власова
def Vlasov():
    with open("Res_part_2.3.txt", "a") as Res:
        print("АБСОЛЮТНЫЕ КООРДИНАТЫ", file=Res)
    ...
    while theta <= theta_max:
        ...
        _B_ = 4 * nu_theta ** 4 - 4 * omega ** 2 * nu_theta ** 2 + (aleph * alpha_0 * omega_lin ** 2 / theta_rad) ** 2

        if _B_ > 0:
            if _A_ + _B_ ** (1 / 2) > 0:
                omega_1 = (_A_ + _B_ ** (1 / 2)) ** (1 / 2)
            else:
                omega_1 = "-"

            if _A_ - _B_ ** (1 / 2) > 0:
                omega_2 = (_A_ - _B_ ** (1 / 2)) ** (1 / 2)
            else:
                omega_2 = "-"

            omega_0 = aleph * alpha_0 * omega_lin ** 2 / (_2_nu_theta * theta_rad)
