...
# Способ В.Г. Власова
def Vlasov():
    with open("Res_part_2.1.txt", "a") as Res:
        print("ОТНОСИТЕЛЬНЫЕ КООРДИНАТЫ. ЛИНЕЙНОЕ СОПРОТИВЛЕНИЕ", file=Res)
    ...
    while theta <= theta_max:
        ...
        # Относительные координаты. Линейное сопротивление
        ...
        _A_ = omega ** 2 - 2 * nu_theta ** 2
        _B_ = (omega ** 2 - 2 * nu_theta ** 2) ** 2 - omega ** 4 * (1 - q ** 2 * alpha_m ** 2 / theta_rad ** 2)
        _C_ = 1 - q ** 2 * alpha_m ** 2 / theta_rad ** 2

        if _B_ > 0:
            if (_A_ + _B_ ** (1 / 2)) / _C_ > 0:
                omega_1 = ((_A_ + _B_ ** (1 / 2)) / _C_) ** (1 / 2)
            else:
                omega_1 = "-"

            if (_A_ - _B_ ** (1 / 2)) / _C_ > 0:
                omega_2 = ((_A_ - _B_ ** (1 / 2)) / _C_) ** (1 / 2)
            else:
                omega_2 = "-"

            omega_0 = _2_nu_theta / (q * alpha_m) * theta_rad

            with open("Res_part_2.1.txt", "a") as Res:
                print(theta, omega_0, omega_1, omega_2, sep="\t", file=Res)
        else:
            omega_1 = "-"
            omega_2 = "-"
